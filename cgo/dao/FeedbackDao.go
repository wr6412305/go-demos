...
func (p *FeedbackDao) Insert(feedback *entity.Feedback) int64 {
	result, err := cgo.DB.Exec(
		"insert into feedback(`user_id`,`title`,`content`,`create_time`) value(?,?,?,?)",
		feedback.UserID, feedback.Title, feedback.Content, feedback.CreateTime)
	if err != nil {
		log.Println(err)
		return 0
	}
...
func (p *FeedbackDao) SelectFeedbackByUserId(id uint) []*model.FeedbackResp {
	...
	for rows.Next() {
		...
		err := rows.Scan(&temp.Feedback.ID, &temp.Feedback.UserID, &temp.Feedback.Title,
			&temp.Feedback.Content, &temp.Feedback.CreateTime, &temp.Picture.ID,
			&temp.Picture.FeedbackID, &temp.Picture.Address, &temp.Picture.CreateTime)
		if err != nil {
			log.Println(err)
			continue
		}
