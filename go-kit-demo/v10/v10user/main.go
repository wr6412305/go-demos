...
func main() {
	...
	go func() {
		...
		server := v10service.NewService(utils.GetLogger(), count, histogram, tracer)
		endpoints := v10endpoint.NewEndPointServer(server, utils.GetLogger(), golangLimit, tracer)
		grpcServer := v10transport.NewGRPCServer(endpoints, utils.GetLogger())
