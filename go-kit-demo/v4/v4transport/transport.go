...
// NewHTTPHandler ...
func NewHTTPHandler(endpoint v4endpoint.EndPointServer, log *zap.Logger) http.Handler {
	options := []httptransport.ServerOption{
		httptransport.ServerErrorEncoder(func(ctx context.Context, err error, w http.ResponseWriter) {
			log.Warn(fmt.Sprint(ctx.Value(v4service.ContextReqUUID)), zap.Error(err))
		...
		httptransport.ServerBefore(func(ctx context.Context, request *http.Request) context.Context {
			...
			ctx = context.WithValue(ctx, v4service.ContextReqUUID, UUID)
...
func decodeHTTPLoginRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	var login v4service.Login
	err := json.NewDecoder(r.Body).Decode(&login)
	if err != nil {
		return nil, err
	}
	utils.GetLogger().Debug(fmt.Sprint(ctx.Value(v4service.ContextReqUUID)), zap.Any(" 开始解析请求数据", login))
	return login, nil
}
...
func decodeHTTPADDRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	...
	utils.GetLogger().Debug(fmt.Sprint(ctx.Value(v4service.ContextReqUUID)), zap.Any(" 开始解析请求数据", in))
	return in, nil
}

func encodeHTTPGenericResponse(ctx context.Context, w http.ResponseWriter, response interface{}) error {
	utils.GetLogger().Debug(fmt.Sprint(ctx.Value(v4service.ContextReqUUID)), zap.Any("请求结束封装返回值", response))
