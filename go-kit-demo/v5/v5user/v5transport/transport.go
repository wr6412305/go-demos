...
// NewGRPCServer ...
func NewGRPCServer(endpoint v5endpoint.EndPointServer, log *zap.Logger) pb.UserServer {
	options := []grpctransport.ServerOption{
		grpctransport.ServerBefore(func(ctx context.Context, md metadata.MD) context.Context {
			ctx = context.WithValue(ctx, v5service.ContextReqUUID, md.Get(v5service.ContextReqUUID))
			return ctx
		}),
